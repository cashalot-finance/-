...
class PolymarketDailyEnv(gym.Env):
    ...
    def __init__(self, dataset: DailyDataset, config: DataConfig, initial_balance: float = 1.0):
        ...
        # Observation: price, days_to_expiry, volume, n_outcomes, rank, balance
        self.observation_space = gym.spaces.Box(
            low=np.array([0, 0, 0, 2, 1, 0], dtype=np.float32),
            high=np.array([1, 30, 1e9, 50, 10, 1000], dtype=np.float32),
        )
    ...
    def step(self, action: int):
        ...
        reward = 0.0
        if action == 1:
            daily_return = compute_reward(sample, self.config)
            self.balance *= max(1.0 + daily_return, 0.0)
            reward = float(np.log1p(daily_return))
